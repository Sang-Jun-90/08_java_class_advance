...
class Member {
	...
	void printData() {
		System.out.println(custno + "\t" + custname + "\t" + custname + "\t" + totalBuy);
		System.out.println("--------------------------------");	
	}
...
public class TestClass {
	...
	public static void main(String[] args) {
		...
		mg.init();
		int memberSize = mg.memberList.size(); 
		int moneySize = mg.moneyList.size();
		...
		for (int i = 0; i < moneySize; i++) {
			for (int j = 0; j < memberSize; j++) {
				if (mg.moneyList.get(i).custno == mg.memberList.get(j).getCustno() ) {
					mg.memberList.get(j).totalBuy += mg.moneyList.get(i).price;
				}
			}
		}
		...
		while(k < memberSize) { // 0,1,2,3,4,5
			for (int i = k ; i < memberSize; i++) {  // k = 1 // i - 1,2,3,4,5
				if (mg.memberList.get(k).totalBuy < mg.memberList.get(i).totalBuy) {
					Member tempI = mg.memberList.get(i);
					Member tempK = mg.memberList.get(k);
		
					mg.memberList.set(k,tempI);	
					mg.memberList.set(i,tempK);	
				}
			}
			k++;
		}
		...
		for (int i = 0; i < memberSize; i++) {
			if (mg.memberList.get(i).totalBuy != 0) {
				mg.memberList.get(i).printData();
			}
		}
...
//		int k = 0;
//		int maxNum = 1000000; // 4900
//		
//		while (k < 6) {
//			
//			int max = 0;
//			for (int i = 0; i < memberSize; i++) {
//				if (max < mg.memberList.get(i).totalBuy && mg.memberList.get(i).totalBuy < maxNum) {
//					max = mg.memberList.get(i).totalBuy;
//				}
//			}
//
//			for (int i = 0; i < memberSize; i++) {
//				if (mg.memberList.get(i).totalBuy == max && mg.memberList.get(i).totalBuy !=0) {
//					mg.memberList.get(i).printData();
//					maxNum = max;
//				}
//			}
